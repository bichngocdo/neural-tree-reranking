...
import rcnnrr.nn
from rcnnrr.rcnn.rcnn import RCNNCell, RCNNDropoutWrapper, rcnn
...
class RCNNReranker(object):
    ...
    def _build(self, args, is_training):
        ...
        with tf.variable_scope('input_layers', reuse=tf.AUTO_REUSE):
            ...
            if args.word_dim != args.hidden_dim:
                e_word = tf.layers.dense(e_word,
                                         units=args.hidden_dim,
                                         use_bias=False,
                                         activation=None,
                                         kernel_regularizer=tf.contrib.layers.l2_regularizer(args.l2))

            batch_size, max_length, dim = tf.unstack(tf.shape(e_word))
            ...
            e_word = tf.reshape(tf.tile(e_word, (1, num_trees, 1)),
                                (batch_size * num_trees, max_length, dim))
            x_tag_ = tf.reshape(tf.tile(x_tag, (1, num_trees)),
                                (batch_size * num_trees, max_length))

        with tf.variable_scope('hidden_layers', reuse=tf.AUTO_REUSE):
            rcnn_cell = RCNNCell(args.hidden_dim, args.distance_dim, args.max_length, args.no_tags,
                                 regularizer=tf.contrib.layers.l2_regularizer(args.l2))
            if is_training:
                rcnn_cell = RCNNDropoutWrapper(rcnn_cell, keep_prob=1 - args.recursive_dropout)
            e_phrase, subtree_scores = rcnn(rcnn_cell, e_word, x_tag_, x_head_, x_children_)
            e_phrase = tf.reshape(e_phrase, (batch_size, num_trees, max_length, dim))
        ...
        with tf.variable_scope('output_layers', reuse=tf.AUTO_REUSE):
            ...
            # tree_scores = self.alpha * tree_scores + (1. - self.alpha) * x_score
