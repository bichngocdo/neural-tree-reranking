...
import rcnnrr.reranker_gcn.data_reader as data_reader
import rcnnrr.reranker_gcn.data_writer as data_writer
from rcnnrr.data.dataset import Dataset, SimpleBatch
from rcnnrr.reranker_gcn.data_converter import DataConverter
from rcnnrr.reranker_gcn.data_loader import DataLoader
from rcnnrr.reranker_gcn.evaluator import Stats
from rcnnrr.reranker_gcn.network import GCNReranker
...
class Experiment(object):
    ...
    def init_network(self, session):
        self.network = GCNReranker(self.args)
        self.network.initialize_global_variables(session)
    ...
    def __down_sample(self, batch):
        new_batch = list()
        for i in range(6):
            new_batch.append(batch[i])

        all_ids = list()
        ns = [len(item) for item in batch[6]]
        for n, labels in zip(ns, batch[13]):
            ids = list(range(n))
            random.shuffle(ids)
            gold_id = [id for id in range(len(labels)) if labels[id] == 1][0]
            ids = ids[:self.args.down_sampling]
            if gold_id not in ids:
                ids = ids[:-1] + [gold_id]
            all_ids.append(ids)

        for i in range(6, len(batch)):
            l = list()
            for ids, items in zip(all_ids, batch[i]):
                l.append([items[j] for j in ids])
            new_batch.append(l)
    ...
    def __train(self, train_dataset, dev_dataset, interval,
        ...
        for batch in batches:
            batch = self.__down_sample(batch)
...
def train(args):
    ...
    train_raw_data = data_reader.read(args.train_file, add_oracle=args.include_train_oracles,
                                      keep_all=False, is_training=True)
    print('Load data from file %s, no. sentences: %d' % (args.train_file, len(train_raw_data['words'])))

    dev_raw_data = data_reader.read(args.dev_file, add_oracle=args.include_eval_oracles,
                                    keep_all=True, is_training=False)
    print('Load data from file %s, no. sentences: %d' % (args.dev_file, len(dev_raw_data['words'])))

    if args.test_file is not None:
        test_raw_data = data_reader.read(args.test_file, add_oracle=args.include_eval_oracles,
                                         keep_all=True, is_training=False)
...
def evaluate(args):
    ...
    dev_raw_data = data_reader.read(args.test_file, add_oracle=args.include_oracles,
                                    keep_all=True, is_training=False)
...
if __name__ == '__main__':
    ...
    group.add_argument('--train_batch_size', type=int, default=5,
                       help='mini-batch size in number of sentences')
    group.add_argument('--validate_batch_size', type=int, default=5,
                       help='mini-batch size in number of sentences')
    group.add_argument('--eval_batch_size', type=int, default=5,
                       help='mini-batch size in number of sentences')
    ...
    group.add_argument('--down_sampling', type=int, default=10,
                       help='down sampling the training set')
    ...
    group.add_argument('--lstm_hidden_dim', type=int, default=200,
                       help='LTSM hidden dimension')
    group.add_argument('--num_mlps', type=int, default=1,
                       help='number of MLP layers')
    group.add_argument('--mlp_dim', type=int, default=200,
                       help='MLP dimension')
    group.add_argument('--num_gcns', type=int, default=1,
                       help='number of GCN layers')
    group.add_argument('--gcn_hidden_dim', type=int, default=200,
                       help='GCN hidden dimension')
    group.add_argument('--use_characters', type=str2bool, default=False,
    ...
    group.add_argument('--loss', choices=['ranking_max', 'ranking_all', 'ranking_all_orders'],
    ...
    group.add_argument('--structured_margin', action='store_true',
                       help='structured margin')
    group.add_argument('--margin_discount', type=float, default=1.,
    ...
    group.add_argument('--gcn_dropout', type=float, default=0.,
                       help='recurrent dropout probability')
